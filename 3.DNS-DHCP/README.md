...
- Note: When setting up **Active Directory Domain Services (AD DS)**,
the DNS role is installed by default.  
By default, only **Forward Lookup Zones** are created automatically.  
If you need **Reverse Lookup Zones** (to resolve IP addresses back to hostnames),
...
- Verified that a Windows 11 client successfully joined the domain.

![alt text](screenshots/10-successfully-joined-DC.png)

**Confirmed the client received a DHCP-assigned IP from the server (ipconfig /all).**
...
- Ensured name resolution worked using nslookup 9quid.local.
